...
namespace _scripts.Movement
{
    public class ArmSwingMotion : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (!CheckIfGrabbing(LeftController))
            {
        ...
        private bool CheckIfGrabbing(XRController xrController)
        {
            xrController.inputDevice.IsPressed(GrabButton, out var isActivated);
            return isActivated;
        }
    }
}